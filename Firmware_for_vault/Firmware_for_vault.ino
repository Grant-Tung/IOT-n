...
0x57,0x2d,0x24,0x17,0x74,0xec,0x47,0xegit add .
git commit -m "Ignore VS temp files and upload firmware"
...
void call_main_menu() {
  ...
  for (int i = 0; i < 320; i++) {
    for (int j = 0; j < 40; j++) {
      tft.drawPixel(i, j + 10, Beirut[i][j]);
    }
  }
  curr_key = 0;
  main_menu(curr_key);
...
void loop() {
  ...
  delay(50); // <-- Đã fix lỗi thiếu dấu chấm phẩy ở đây
}
